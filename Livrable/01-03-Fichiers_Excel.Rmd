...
#### Lecture des fichiers au format xls

Pour lire un fichier au format **xls**, une solution est d'utiliser le package `readxl` développé par Hadley Wickham. Ce package ne dépend d'aucune installation java sur poste. 

Par exemple, le fichier des populations des quartiers de la politique de la ville 2015 est disponible sur le site insee.fr à l'adresse suivante : <https://insee.fr/fr/statistiques/fichier/2500431/pop_qp_2013.xls>

Dans un premier temps, il faut enregistrer le fichier puis le lire avec la fonction `read_xls` du package `readxl`.

```{r,eval=FALSE}
library(readxl)

pop_qpv15 <- read_xls(path = "pop_qp_2013.xls",skip = 8)
```

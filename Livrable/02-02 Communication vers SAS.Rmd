...
Le package **Hmisc** offre une fonction `sas.get` qui permet de lire des tables SAS en dialoguant avec une session SAS lancée depuis R. Pour utiliser la fonction, il faut donc savoir où se trouve installé le programme SAS. La solution est écartée n raison de sa dépendance à la présence du logiciel SAS.

```{r eval=FALSE}
library(Hmisc)
tbl <- sas.get("U:/R", "bsa2003"
...
- Les éventuels labels de variable sont conservés.

## Le package **foreign**

Le package  **foreign** offre une fonction  `read.ssd` qui permet de lire des tables SAS en dialoguant avec une session SAS lancée depuis R. Pour utiliser la fonction, il faut donc savoir où se trouve installé le programme SAS. La solution est écartée en raison de sa dépendance à la présence du logiciel SAS.
...
Remarques :<br>
- Les données de type caractère sous SAS sont transformées en facteurs sous R.<br>
- Les éventuels labels de variable ne sont pas conservés.

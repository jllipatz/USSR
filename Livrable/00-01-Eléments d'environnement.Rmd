...
Avec le package **dplyr** vient le package **margittr** qui introduit une toute autre notation pour ces appels de fonction imbriqués. L'opérateur `%>%` permet de spécifier une cascade d'appels de fonction dans l'ordre où les opérations vont être réalisées. Il suffit juste d'indiquer quel rôle va jouer le résultat d'un calcul dans l'opération suivante et de préciser les différents appels dans l'ordre chronologique :
```{r}
mtcars %>% group_by(.,cyl) %>% summarise(.,n=n()) %>% arrange(.,desc(n))
```
Un petit raffinemment peut être apporté quand le résultat d'un calcul va devenir le remier argument pour le calcul suivant. Dans ce cas les points sont facultatifs et on peut écrire :
```{r}
mtcars %>% group_by(cyl) %>% summarise(n=n()) %>% arrange(desc(n))
```
On peut noter le gain de lisibilité par rapport à l'écriture standard : chaque étape se détache bien ce qui, d'ailleurs, peut être très utile pour tester une séquence d'appels de fonctions en sélectionnant et exécutant seulement les première étapes pour vérfier que le résultat obtenu est conforme avant de prendre en compte l'étape suivante.

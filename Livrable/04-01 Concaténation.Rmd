...
La solution basée sur du R de base est écartée car elle est jugée trop permissive pour la concaténation de tables avec un nombre de lignes différent et trop compliquée pour la concaténation de listes. 

```{r}
resultat <- cbind(table_un, table_deux, couleurs = colonne)
```

Avec la fonction cbind il n'y a pas de message d'erreur (ni d'avertissement) si on veut concaténer 2 tables dont le nombre de ligne de la table la plus petite est un multiple de l'autre.

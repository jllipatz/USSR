...
Présentons les opérateurs permettant d'exprimer une condition logique.
On pourra les utiliser notament afin de ne retenir que certaines variables ou certaines observations dans la base de données.
...
On sépare les différents éléments de la liste avec une virgule

**Dans une liste :**   %in% c(…) 

On combine les opérateurs "different de" et "dans la liste" 

**X N'est pas dans la liste :**  !(x %in% c(…))
...
Les lignes sont les observations, ici les mesures pour une fleur.
Les variables de longueur et de largeur sont numériques.
La variable d'espèce contient 3 modalités : setosa, versicolor et virginica.

...
```
Pour avoir un comportement uniforme, il faut donc présenter les choses de manière à ce que le calcul se fasse en deux temps : un premier de substitution des valeurs des paramètres, un second d'évaluation selon le processus standard. 
...
La fonction `glue`du package de même nom est une généralisation de la saisie de chaînes de caractères de R. En R de base l'utilisation  de \\ signifie l'inclusion d'un caractère spécial dont l'abbréviation suit le \\. La foncton glue rajoute la notation `{...}` qui va dire qu'il faut évaluer ce qu'il y a entre accolades et inclure la chaîne de caractères correspondantes. On peut y mettre un nom de variable ou une expression complète.
```{r echo=TRUE,message=FALSE}
library(glue)
library(sqldf)
...
Cette solution n'a pas été retenue en raison de son adhérence totale au 'tidyverse' : seuls les packages de cet ensemble sont susceptibles de reconnaître la notation `UQ` (ce n'est pas une fonction) qui permet à la fonction `group_by` d'interpréter ce qu'on lui passe comme une référence devant être évaluée.

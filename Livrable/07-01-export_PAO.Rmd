...
À l'heure actuelle, les données sont envoyées à la PAO dans un tableur **calc**. Ils doivent respecter un certain style de mise en forme et les graphiques doivent être générés par calc. L'inconvenient de cette méthode est qu'il est difficil d'automatiser la création de ce fichier et des graphiques correspondant. Au moindre changement en amont, il faut mettre à jour le fichier manuellement ce qui occasionne une perte de temps et l'apparition de coquilles (erreur de copier-coller par exemple). Ce document présente une méthode spécifique permettant d'automatiser la chaine de production des fichiers à destination de la PAO. Les graphiques sont réalisés directement sous R ce qui permet de profiter des fonctionnalités de packages tel que `ggplot2`. 
